...
fn flow_start_step_name<Fs: adk_io::FileSystem>(fs: &Fs, flow_dir: &Path) -> Option<String> {
    let yaml = read_yaml_mapping(fs, &flow_dir.join("flow_config.yaml"))?;
    yaml.get("start_step")
...
impl DiscoverResources for FlowStep {
    fn discover_resources<Fs: adk_io::FileSystem>(fs: &Fs, base_path: &Path) -> Vec<String> {
        ...
        let mut out = Vec::new();
        if let Some(flow_dirs) = sorted_read_dir(fs, &flows_path) {
            for flow_dir in flow_dirs {
                if !is_dir(fs, &flow_dir) {
                    continue;
                }
                let steps_path = flow_dir.join("steps");
                if let Some(files) = sorted_read_dir(fs, &steps_path) {
                    let start_step = flow_start_step_name(fs, &flow_dir);
    ...
    fn discover_resources<Fs: adk_io::FileSystem>(fs: &Fs, base_path: &Path) -> Vec<String> {
        ...
        if let Some(flow_dirs) = sorted_read_dir(fs, &flows_path) {
            for flow_dir in flow_dirs {
                ...
                if let Some(files) = sorted_read_dir(fs, &function_steps_path) {
                    for f in files {
